...
import { Cpu, Thermometer, Hash, Palette, RotateCcw, Check, AlertCircle, Info, X, Plus, Pencil, Trash2, Sparkles, Languages } from 'lucide-react'
              ...
              <div className="space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-medium">{t('settings.api.myModels')}</label>
                    <button
                      onClick={startAdd}
                    ...
                    >
                      <Plus size={14} /> {t('settings.api.addModel')}
                    </button>
                  </div>
                        ...
                        >
                          <button onClick={() => settings.activateCustomModel(m.id)} className="flex-1 min-w-0 text-left">
                            <div className="flex items-center gap-2">
                              ...
                              {active && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-md-primary text-md-onPrimary flex-shrink-0">{t('settings.api.currentBadge')}</span>}
                            </div>
                            <div className="text-xs text-dark-onSurfaceVariant truncate mt-0.5">{m.model} · {m.baseUrl}</div>
                          </button>
                          <button onClick={() => startEdit(m)} className="w-7 h-7 flex items-center justify-center rounded-md3-xs text-dark-onSurfaceVariant hover:bg-dark-surfaceContainerHighest transition-colors flex-shrink-0" aria-label={t('settings.api.edit')}>
                            <Pencil size={13} />
                          </button>
                          <button onClick={() => settings.removeCustomModel(m.id)} className="w-7 h-7 flex items-center justify-center rounded-md3-xs text-dark-onSurfaceVariant hover:text-md-error hover:bg-md-error/10 transition-colors flex-shrink-0" aria-label={t('settings.api.delete')}>
                            <Trash2 size={13} />
                          </button>
                        </div>
                      )
                  ...
                  {editing !== null && (
                    <div className="mt-3 p-4 rounded-md3-md border border-md-primary/20 bg-dark-surfaceContainerHigh space-y-3">
                      <div className="text-sm font-medium">{editing === 'new' ? t('settings.api.addTitle') : t('settings.api.editTitle')}</div>
                      {/* 推荐模板 */}
                      ...
                      <input
                        ...
                        placeholder={t('settings.api.modelIdPlaceholder')}
                        className="w-full px-3 py-2 bg-dark-surfaceContainerHighest rounded-md3-sm text-sm border border-dark-onSurfaceVariant/10 outline-none focus:border-md-primary/40 transition-colors"
                      />
                      <input
                      ...
                      <div className="flex justify-end gap-2 pt-1">
                        <button onClick={() => { setEditing(null); setForm(emptyForm) }} className="px-3 py-1.5 rounded-md3-sm text-xs text-dark-onSurfaceVariant hover:bg-dark-surfaceContainer transition-colors">{t('common.cancel')}</button>
                        <button
                          onClick={saveModel}
