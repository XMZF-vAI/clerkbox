...
- **局域网与扫码**：弹窗内勾选「允许局域网访问」后服务绑定所有网卡，并在检测到局域网 IP 时生成二维码，手机扫码即刻连接（仅在开启局域网且探测到网卡地址时显示，绝不指向 localhost）
- **服务器自动启动**：设置环境变量 `CLERKBOX_WEBUI_AUTO=1` 后启动应用，WebUI 自动开启并在控制台打印访问地址（自动启动默认仅绑定本机，如需远程请先在桌面端开启局域网访问）
...
- **窄屏移动端布局**：网页在小屏自动切换移动端 UI（大触控按钮、底部抽屉菜单、虚拟键盘避让），便于在手机上操作
- **浏览器文件**：本机或第三设备 WebUI 都可上传单个文件到主机（每个文件默认不超过 10MB），并在主机目录树中选择工作目录

### 注意事项

- WebUI 默认仅监听 `127.0.0.1` 的随机端口，仅供本机访问；勾选「允许局域网访问」后才绑定 `0.0.0.0`，同一局域网内设备可访问；公网部署建议配合反向代理与 HTTPS
